...
GenOutline.prototype.style = function(nr){
...
//   endShape();
// }
GenOutline.prototype.createOutline = function(marge,cornersteps,size, step){
...
GenOutline.prototype.createOutline1 = function(marge,cornersteps,size, step){
  this.outline = [];
  var m = marge;
  ...
  for(var st = 0 ; st < size; st+= step){
      append(this.outline ,createVector(minx,maxy+(m+st)));
    for(var s = 0; s <=(m+st); s +=parseInt((m+st)/cornersteps)){
      append(this.outline ,createVector(maxx + s,maxy+((m+st) - s)));
    }
    for(var s = 0; s <=(m+st); s +=parseInt((m+st)/cornersteps)){
      append(this.outline ,createVector(maxx + ((m+st)-s),miny - s));
    }
    for(var s = 0; s <=(m+st); s +=parseInt((m+st)/cornersteps)){
      append(this.outline ,createVector(minx - s,miny - ((m+st)-s)));
    }
    for(var s = 0; s <(m+st); s +=parseInt((m+st)/cornersteps)){
      append(this.outline ,createVector(minx - ((m+st) - s),maxy + s));
    }
  }
}
GenOutline.prototype.drawOutline = function(){
  this.style(0);
  for(var i= 1; i < this.outline.length; i++){
    line(this.outline[i-1].x, this.outline[i-1].y,this.outline[i].x, this.outline[i].y);
  }
}
GenOutline.prototype.genOutline = function(offset){
  var x, y;
